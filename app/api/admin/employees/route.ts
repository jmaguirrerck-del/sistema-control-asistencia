...
export async function GET(){
  ...
  const rows=await sql`
    SELECT e.id,e.last_name,e.first_name,e.dni,e.employment,e.active,e.seniority_date::text,e.seniority_notes,(e.pin_hash IS NOT NULL) AS pin_configured,e.force_pin_change,e.pin_changed_at,e.pin_change_source,e.pin_reset_count,e.temporary_pin_expires_at,
...
export async function POST(request:Request){
  ...
  try{
    await sql`INSERT INTO employees(id,last_name,first_name,dni,employment,seniority_date,seniority_notes,pin_hash,pin_lookup,force_pin_change,pin_changed_at,pin_change_source,temporary_pin_expires_at,active) VALUES (${id},${lastName},${firstName},${dni},${employment},${seniorityDate||null}::date,${seniorityNotes||null},${hash},${lookup},${Boolean(pin)},${pin?new Date():null},${pin?"ADMIN":null},${pin?new Date(Date.now()+7*24*60*60*1000):null},TRUE)`;
