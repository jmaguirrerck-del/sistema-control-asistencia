...
export async function POST(request:Request){
  ...
  const rows=await sql`INSERT INTO leave_records(employee_id,leave_type,leave_type_id,date_from,date_to,observation,computed_days,warning_text,created_by) VALUES(${employeeId},${leaveType},${typeId},${dateFrom}::date,${dateTo}::date,${observation||null},${computed},${warning||null},${session!.email}) RETURNING id`;
...
export async function DELETE(request:Request){
  await ensureV13Schema();const session=await getAdminSession();if(!canManageLicenses(session))return NextResponse.json({error:"No autorizado"},{status:403});const id=Number(new URL(request.url).searchParams.get("id"));if(!Number.isInteger(id))return NextResponse.json({error:"ID inválido"},{status:400});const sql=db();const prev=(await sql`SELECT * FROM leave_records WHERE id=${id}`)[0];if(!prev)return NextResponse.json({error:"Registro inexistente"},{status:404});
