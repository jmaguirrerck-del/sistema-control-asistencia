...
import { findEmployeeByPin, registerEntry, registerExit, registerReentry, todayEmployeeContext, validateLocation, validateQr } from "@/lib/attendance";
...
export async function POST(request:Request){
  ...
  if(action!=="ENTRY"&&action!=="EXIT"&&action!=="REENTRY")return NextResponse.json({error:"Acción de marcación inválida."},{status:400});
  ...
  try{
    ...
    if(action==="REENTRY"){
      const day=await registerReentry({employeeId:String(employee.id),location:locationInput,distance:Number(geo.distance)});await sql`UPDATE qr_tokens SET used_count=used_count+1 WHERE id=${qr.id}`;
      return NextResponse.json({kind:"REENTRY",name:`${employee.last_name}, ${employee.first_name}`,serverTime:serverTime(),lateMinutes:Number(day.late_minutes||0)});
    }
    ...
    const code=error instanceof Error?error.message:"";const messages:Record<string,string>={ENTRY_EXISTS:"La entrada de hoy ya fue registrada.",EXIT_EXISTS:"La salida ya fue registrada. Si regresaste a la oficina, registrá primero el reingreso.",REENTRY_NOT_ALLOWED:"No hay una salida intermedia pendiente de reingreso.",NO_ENTRY:"No existe una entrada previa para registrar la salida.",NO_SCHEDULE:"Hoy no corresponde prestación.",ON_LEAVE:"Existe una novedad vigente para hoy."};return NextResponse.json({error:messages[code]||"No se pudo registrar la marcación."},{status:409});
  }
}
