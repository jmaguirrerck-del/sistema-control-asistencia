...
type Status = { employee:{id:string;name:string;dni:string}; schedule:string; action:"ENTRY"|"EXIT"|"REENTRY"|"DONE"; lastMark?:string|null; message?:string; lateMinutes?:number; mustChangePin?:boolean };
type Success = { kind:"ENTRY"|"EXIT"|"REENTRY"; name:string; serverTime:string; entry?:string; exit?:string; lateMinutes?:number; compensationMinutes?:number; pendingMinutes?:number };
...
export default function MarkClient({ token }: { token: string }){
  ...
  useEffect(()=>{
    try{
      ...
      const signature=[
        ...
        String(Math.round((window.devicePixelRatio||1)*100)/100),
  ...
  if(success)return <main className="mark-shell"><div className="mark-card stack" style={{textAlign:"center"}}><div className="status-icon good">✓</div><h1 className="mark-title">{success.kind==="ENTRY"?"ENTRADA REGISTRADA CORRECTAMENTE":success.kind==="REENTRY"?"REINGRESO REGISTRADO CORRECTAMENTE":"SALIDA REGISTRADA CORRECTAMENTE"}</h1><div className="mark-time">{success.serverTime}</div><strong>{success.name}</strong><div className="notice good">Ubicación verificada · registro guardado en el servidor</div>{success.kind==="ENTRY"?<div className="muted">Próxima acción: registrar salida cuando te retires de la oficina.</div>:success.kind==="REENTRY"?<div className="muted">El intervalo de salida quedó registrado y pendiente de clasificación por Administración.</div>:<div className="grid grid-2" style={{textAlign:"left"}}><div className="kpi info"><div className="kpi-label">Compensación</div><div className="kpi-value">{success.compensationMinutes||0} min</div></div><div className="kpi info"><div className="kpi-label">Saldo pendiente</div><div className="kpi-value">{success.pendingMinutes||0} min</div></div></div>}<button className="btn btn-secondary" onClick={()=>window.location.reload()}>Finalizar</button></div></main>;
  ...
  return <main className="mark-shell"><div className="mark-card stack"><div style={{textAlign:"center"}}><div className="brand-kicker">Dirección de Gestión Escolar</div><h1 className="mark-title">Registro de asistencia</h1><div className="mark-time">{currentTime}</div></div>
    ...
    {status&&!changePinMode&&<div className="stack"><div className="notice info"><strong>{status.employee.name}</strong><br/>DNI {status.employee.dni}<br/><span className="muted">Horario de hoy: {status.schedule}</span></div>{status.lastMark&&<div className="notice info">Última marcación: {status.lastMark}</div>}{pinMessage&&<div className="notice good">{pinMessage}</div>}{error&&<div className="notice bad">{error}</div>}{status.action==="DONE"?<div className="notice good">La jornada fue cerrada automáticamente. Si necesitás corregirla, debe intervenir Administración.</div>:<><button className={status.action==="ENTRY"||status.action==="REENTRY"?"btn btn-success":"btn btn-primary"} onClick={mark} disabled={busy}>{busy?"Registrando…":status.action==="ENTRY"?"REGISTRAR ENTRADA":status.action==="REENTRY"?"REGISTRAR REINGRESO":"REGISTRAR SALIDA"}</button>{status.action==="REENTRY"&&<div className="notice info">Si ya terminaste tu jornada, no vuelvas a marcar. Usá <strong>Reingreso</strong> solamente cuando regreses a la oficina después de una salida intermedia.</div>}</>}<button className="btn btn-secondary" type="button" onClick={()=>{setChangePinMode(true);setError("");setPinMessage("");}}>Cambiar mi PIN</button><div className="notice info">Este dispositivo está asociado a {status.employee.name}. Para cambiar de dispositivo o de agente, debe intervenir Administración.</div></div>}
